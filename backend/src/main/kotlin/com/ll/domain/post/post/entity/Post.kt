...
class Post : BaseTime {
    ...
    private fun processGenFile(
        ...
        val originalFileName = com.ll.standard.util.Ut.file.getOriginalFileName(filePath)
        val metadataStrFromFileName = com.ll.standard.util.Ut.file.getMetadataStrFromFileName(filePath)
        val fileExt = com.ll.standard.util.Ut.file.getFileExt(filePath)
        val fileExtTypeCode = com.ll.standard.util.Ut.file.getFileExtTypeCodeFromFileExt(fileExt)
        ...
        if (com.ll.standard.util.Ut.str.isNotBlank(metadataStrFromFileName)) {
            metadataStr = if (com.ll.standard.util.Ut.str.isNotBlank(metadataStr))
                "$metadataStr&$metadataStrFromFileName"
            else
                metadataStrFromFileName
        }

        val fileName = if (isModify) com.ll.standard.util.Ut.file.withNewExt(oldPostGenFile!!.fileName, fileExt) else UUID.randomUUID()
            .toString() + "." + fileExt
        val fileSize = com.ll.standard.util.Ut.file.getFileSize(filePath)
        ...
        genFile.fileDateDir = com.ll.standard.util.Ut.date.getCurrentDateFormatted("yyyy_MM_dd")
        ...
        if (isModify) {
            com.ll.standard.util.Ut.file.rm(genFile.filePath)
        }

        com.ll.standard.util.Ut.file.mv(filePath, genFile.filePath)
    ...
    fun deleteGenFile(postGenFile: PostGenFile) {
        com.ll.standard.util.Ut.file.rm(postGenFile.filePath)
